...
| STEP-2.9 | DONE | Production audit found 7 payments with business_date differing from canonical collection rule | Reused `getPaymentCollectionTime`, `computeBusinessDate`, tenant connection pattern from `tenant-migration-runner.js`, and `AuditLog` model | Created maintenance repair script (`src/scripts/repair-payment-business-dates.js`) with preview-by-default, `--apply --confirm` safety guard, closed ledger day protection (needs manual adjustment), audit logging, and `allowBusinessDateRepair` hook bypass | `gymsera_be/tests/integration/repair-payment-business-dates.test.js` | 1126343 | pending |
